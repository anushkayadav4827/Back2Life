...
- **Native mobile app (iOS/Android), built only after the responsive web app is complete and demo-ready.** The backend (API Gateway + Lambda) is already device-agnostic JSON-over-HTTPS, so the app phase reuses it unchanged — only a native frontend needs to be built.
